...
export default function ContactUsClient({ contacts }: Props) {
  ...
  return (
    ...
    {contacts && contacts.length > 0 && contacts.map((val: contacts, idx: number) => 
      <div key={idx} className={`${idx !== contacts.length - 1 && 'pb-4'}`}>
        ...
        activeMapIndex === idx && 'border-secondary'}`} onMouseEnter={() => setActiveMapIndex(idx)} onClick={handleScrollToTop}>
          {/* Left Side: Text */}
          <div className="p-4">
            <div className="pb-4">
